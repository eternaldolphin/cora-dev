...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_ori = (q @ k.transpose(-2, -1)) * self.scale# [1, 12, 197, 64]@[1, 12, 64, 197]->[1, 12, 197, 197]
        attn_ori = attn_ori.softmax(dim=-1)# [1, 12, 197, 197]
        attn_ori = self.attn_drop(attn_ori)# [1, 12, 197, 197]
        ...
        attn = (q @ k.transpose(-2, -1)) * scale#[1, 12, 197, 197]
        attn = (attn).softmax(dim=-1)
        attn = self.attn_drop(attn)

        x_ori = (attn_ori @ v).transpose(1, 2).reshape(B, N, C)# [1, 12, 197, 197]@[1, 12, 197, 64]->[1, 197, 12, 64]->[1, 197, 768]
        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # clip_surgery [1, 12, 197, 197]@[1, 12, 197, 64]->[1, 197, 768]
        #x = v.transpose(1, 2).reshape(B, N, C) # mask_clip
        x = self.proj_drop(self.proj(x))# [1, 197, 768]
        x_ori = self.proj_drop(self.proj(x_ori))# [1, 197, 768]
        return [x, x_ori]
...
class CSModifiedResNet(nn.Module):
    """
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        if isinstance(self.attn, Attention):
            x = x.transpose(0, 1)
            x, x_ori = self.attn(x)# [1, 197, 768] [1, 197, 768]
            return [x.transpose(0, 1), x_ori.transpose(0, 1)]# [197, 1, 768] [197, 1, 768]
        else:
        ...
        if isinstance(self.attn, Attention):
            ...
            # start of dual path
                ...
                if isinstance(x_res, list):
                    x_res, x_ori_res = x_res# [197, 1, 768] [197, 1, 768]
                    x_ori = x + x_ori_res# [197, 1, 768]
                    x_ori = x_ori + self.mlp(self.ln_2(x_ori))# [197, 1, 768]
                    x += x_res# [197, 1, 768]
                    return [x, x_ori]
